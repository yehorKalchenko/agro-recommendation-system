...
from app.services.cv.rekognition_adapter import analyse_with_rekognition, RekognitionUnavailable
...
async def extract_visual_features(
    ...
    if images_bytes:
        ...
        if settings.USE_REKOGNITION:
            try:
                rek_feats = analyse_with_rekognition(images_bytes)
                feats.update(rek_feats)
